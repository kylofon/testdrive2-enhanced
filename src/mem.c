...
bool mem_load_exe(const char *path, char *err, size_t errlen)
{
    ...
    if (image_len != 0x1FCC0 || lin(LOAD_SEG, 0) + image_len > lin(HEAP_BOTTOM, 0)) {
        set_err(err, errlen, "%s: unexpected load image size %u", path, (unsigned)image_len);
        goto done;
    }
    ...
    /* Identify the build: TD2EGA keeps these strings at fixed DGROUP offsets. */
    if (memcmp(mp(DGROUP, 0x0473), "testdrv2.pes", 12) != 0 ||
        memcmp(mp(DGROUP, 0x651A), "locateshape", 11) != 0 ||
        memcmp(mp(DGROUP, 0x5FE8), "NO ROOM LEFT ON TIMER", 21) != 0) {
        set_err(err, errlen, "%s is not the expected TD2EGA.EXE (EGA build of Test Drive II)", path);
        goto done;
    }

    /* PORT: the copy protection (13a8:002e) is dropped. The file holds its "failed" state; write what a
     * successful check writes (port/spec/platform.md 4.28, game_flow.md 9.3): the three driving-code jump
     * opcodes become JNE (the byte at 13a8:03d2), stage_run returns with RETF, DS:5656 = 0. Game code is
     * ported with the "passed" behaviour anyway; this keeps the image consistent with it. */
    SEGB(0x06C9, 0x424A) = SEGB(0x13A8, 0x03D2);
    SEGB(0x06C9, 0x4275) = SEGB(0x13A8, 0x03D2);
    SEGB(0x06C9, 0x42AD) = SEGB(0x13A8, 0x03D2);
    SEGB(0x06C9, 0x1BF7) = 0xCB;
    DSB(0x5656) = 0;
    ok = true;
done:
    SDL_free(relocs);
...
{
    /* MSC 5 runtime INT 0 handler (13a8:04be): prints R6003 and exits. */
